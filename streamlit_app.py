...
def main():
    ...
    st.markdown(
        """
    ### Problem Statement
        The objective here is to predict the customer satisfaction score for a given order
        """
    )
    st.image(whole_pipeline_image, caption="Whole Pipeline")
    st.markdown(
        """
    Above is a figure of the whole pipeline, we first imgest the data, clean it, train the model and evaluate it. 
        """
    )
    ...
    st.markdown(
        ...
        | Models        | Description   |
        | ------------- | -     |
        | Payment Sequential | Customer may pay an order with more than one payment method. |
    ...
    # Need to complete markdown above
    ...
    if st.button("Predict"):
        ...
        if service is None:
            st.write(
                "No service could be found. The pipeline will run first to create a service"
            )
            run_main()
            ...
            st.success(
                f"Your Customer Satisfactory rate(range between 0 - 5) with given product details are following: {pred}"
            )
    if st.button("Results"):
        st.write("We have experimented with two ensemble and tree based models and compared the prediction")
        ...
        st.write(
            "Following figure show how important each feature is in the model that contribute"
        )
